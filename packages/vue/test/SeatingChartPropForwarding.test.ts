...
// The binding contract (`SEATING_CHART_IDENTITY_PROPS`, `bindSeatingChartHandle`,
// `buildSeatingChartOptions`) is imported REAL — it is the thing the wrapper is
// now a shell over, and faking it would fake away what these tests check. Only
// the chart class is a stub. The module has no runtime imports of its own, so
// pulling it in costs nothing.
